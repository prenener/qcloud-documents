...
<dx-tabs>
::: iOS&Mac\s平台
在 iOS/Mac 平台上，摄像头原生采集的视频格式即是 NV12，原生支持且性能最佳的视频帧格式是CVPixelBufferRef，同时支持I420、OpenGL 2D纹理格式。

#### 示例代码
在 [Demo](https://github.com/LiteAVSDK/TRTC_iOS/tree/main/TRTC-API-Example-OC/Advanced/LocalVideoShare) 中的 [LocalVideoShareViewController.m](https://github.com/LiteAVSDK/TRTC_iOS/blob/main/TRTC-API-Example-OC/Advanced/LocalVideoShare/LocalVideoShareViewController.m) 文件，它展示了如何从一个本地视频文件中读取出 NV12 格式的 PixelBuffer，并通过 SDK 进行后续处理。
...
<tr>
  ...
  <td>uint32_t</td>
  <td>视频数据的长度</td>
  <td>视频数据的长度，单位是字节。对于 i420 而言：length = width * height * 3 / 2；对于 BGRA32 而言：length = width * height * 4。</td>
</tr><tr>
  <td>width</td>
  <td>uint32_t</td>
  <td>视频画面的宽度</td>
  ...
  <td>画面旋转角度</td>
  <td><li>默认不填写。 </li><li>如果需要对画面进行旋转，可以填写<code>TRTCVideoRotation0</code>、<code>TRTCVideoRotation90</code>、<code>TRTCVideoRotation180</code>、<code>TRTCVideoRotation270</code>。SDK  会根据这个值将视频顺时针旋转对应角度。例如一个竖屏画面，传入<code>TRTCVideoRotation90</code>后，SDK 会顺时针旋转90度显示。</li></td>
</tr></tbody></table>

#### 示例代码
在 [Demo](https://github.com/LiteAVSDK/TRTC_Windows/blob/main/TRTC-API-Example-C++/TRTC-API-Example-Qt/src/TestCustomCapture/test_custom_capture.cpp) 文件中有一个叫做 `startCustomVideoData` 的函数，它展示了如何从一个本地视频文件中读取出 I420 格式的 buffer，并通过 SDK 进行后续处理。
...
<dx-tabs>
::: iOS&Mac\s平台
通过 TRTCCloud 的 `setLocalVideoRenderDelegate` 和 `setRemoteVideoRenderDelegate` 可以设置本地和远程画面的自定义渲染回调。

[](id:example_ios)
#### 示例代码
在 [Demo](https://github.com/LiteAVSDK/TRTC_iOS/tree/aa3026c07baeda553aec491702382683d5486a32/TRTC-API-Example-Swift/CustomCapture/testCustomVideo) 中的 [TestRenderVideoFrame.m](https://github.com/LiteAVSDK/TRTC_iOS/blob/aa3026c07baeda553aec491702382683d5486a32/TRTC-API-Example-Swift/CustomCapture/testCustomVideo/TestRenderVideoFrame.m#L39) 文件展示了如何使用该方法。
...
|  参数名称   |         参数类型          |          推荐取值           |                           备注说明                           |
| :---------: | :-----------------------: | :-------------------------: | :----------------------------------------------------------: |
| pixelFormat |   TRTCVideoPixelFormat    | TRTCVideoPixelFormat_I420 | 支持回调 TRTCVideoPixelFormat_I420 或 TRTCVideoPixelFormat_BGRA32 像素格式的视频帧。 |
...
[](id:example_windows)
#### 示例代码
在 [Demo](https://github.com/LiteAVSDK/TRTC_Windows/blob/main/TRTC-API-Example-C++/TRTC-API-Example-Qt/src/TestCustomRender/test_custom_render.cpp) 文件中有一个叫做 `onRenderVideoFrame` 的函数，是自定义视频渲染的回调函数，里面展示如何将视频数据进行绘制显示
...
<ul style="margin:0"><li/><b>iOS&Mac：</b>setAudioFrameDelegate。
